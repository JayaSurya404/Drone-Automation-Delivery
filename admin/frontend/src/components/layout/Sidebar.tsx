...
export const Sidebar: React.FC<SidebarProps> = ({ collapsed, onToggle, mobileOpen = false, onMobileClose }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 z-50 md:z-40 h-screen border-r border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-950/95 shadow-2xl md:shadow-xl backdrop-blur-xl transition-all duration-300 flex flex-col ${
          mobileOpen
            ? 'translate-x-0 w-64'
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-4 border-b border-slate-200 dark:border-slate-800/80">
          ...
          <button
            onClick={onMobileClose}
            className="flex md:hidden h-8 w-8 items-center justify-center rounded-lg border border-slate-200 dark:border-slate-800 text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800"
            title="Close Menu"
          >
            <X className="h-4 w-4" />
          </button>
