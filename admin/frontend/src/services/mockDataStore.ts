      ...
      if (ordersRes.ok) {
        ...
        if (Array.isArray(rawOrders)) {
          this.orders = rawOrders.map((o: any) => ({
            ...
            merchantId: 'M-01',
            merchantName: o.merchantName || 'SkyHub Chinniyampalayam',
            packageId: `PKG-${o.id}`,
            packageName: o.packageName || 'Package Pod',
            packageWeightKg: o.packageWeightKg || 1.2,
            packageDimensions: '25x20x15 cm',
            pickupAddress: o.pickupAddress || 'SkyHub Chinniyampalayam',
            pickupCoords: o.pickupCoords || { lat: 11.0550, lng: 77.0650 },
            destinationAddress: o.destinationAddress || 'Customer Destination',
            destinationCoords: o.destinationCoords || { lat: 11.0550, lng: 77.0650 },
            droneId: o.droneId,
      ...
      if (fleetRes.ok) {
        ...
        if (Array.isArray(rawFleet)) {
          this.drones = rawFleet.map((d: any) => ({
            ...
            location: d.location || { lat: 11.0550, lng: 77.0650, altitude: 0, speed: 0, heading: 0 },
      ...
      if (missionsRes.ok) {
        ...
        if (Array.isArray(rawMissions)) {
          this.missions = rawMissions.map((m: any) => ({
            ...
            orderId: m.orderId,
            droneId: m.droneId,
            pickupAddress: 'SkyHub Chinniyampalayam',
            pickupCoords: { lat: 11.0550, lng: 77.0650 },
            destinationAddress: m.destinationAddress || 'Drop Zone',
            destinationCoords: m.currentCoords || { lat: 11.0550, lng: 77.0650 },
