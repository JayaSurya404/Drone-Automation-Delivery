...
class DroneTrackingService {
  ...
  // Ingest authoritative telemetry from Admin Backend and broadcast to customer subscribers
  public ingestTelemetryUpdate(telemetry: {
    ...
    // Update delivery record in SQLite
    runCommand(`
      UPDATE deliveries SET
        current_latitude = ?,
        current_longitude = ?,
    ...
    `, [
      ...
      telemetry.currentLocation.altitudeMeters,
      telemetry.currentLocation.speedKmh,
      telemetry.currentLocation.bearing,
      telemetry.remainingDistanceKm,
      telemetry.estimatedArrivalMins,
      telemetry.status,
      orderId,
    ]);
