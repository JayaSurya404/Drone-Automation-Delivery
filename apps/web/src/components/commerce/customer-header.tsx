...
export function CustomerHeader() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full border-b border-slate-200 dark:border-slate-800/80 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl transition">
            ...
            <div className="flex items-center gap-6">
              <Link href="/customer" className="flex items-center gap-2 group">
                ...
                <div>
                  ...
                  <span className="hidden sm:inline-block ml-1 text-[10px] uppercase font-bold tracking-widest px-1.5 py-0.5 rounded bg-brand-500/10 text-brand-600 dark:text-brand-400">
                    Store
                  </span>
                </div>
              </Link>
              ...
              <button
                ...
                className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition text-left"
              >
                ...
                <div className="text-xs">
                  ...
                  <p className="font-semibold text-slate-800 dark:text-slate-200 line-clamp-1 max-w-[140px] leading-tight">
            ...
            <div className="flex-1 max-w-lg hidden sm:block">
              <form onSubmit={handleSearchSubmit} className="relative">
                <input
                  ...
                  placeholder="Search groceries, essentials, electronics, emergency kits..."
              ...
              <Link
                ...
                className={`relative flex items-center gap-2 px-3 py-2 rounded-xl transition ${
                  pathname === "/customer/cart"
                    ...
                    : "bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 hover:bg-brand-50 dark:hover:bg-brand-950/40 hover:text-brand-600"
                }`}
                aria-label="Shopping Cart"
              >
                <ShoppingCartIcon size={20} />
                <span className="hidden sm:inline text-xs font-bold">
                  {cartCount > 0 ? `$${((cart?.totalCents || 0) / 100).toFixed(2)}` : "Cart"}
                </span>
                {cartCount > 0 && (
                  <span className="px-1.5 py-0.5 text-[10px] font-bold text-white bg-brand-500 rounded-full">
                    {cartCount}
                  </span>
                )}
              </Link>
